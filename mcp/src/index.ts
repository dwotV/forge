...
async function safeFetch(url: string, init?: RequestInit): Promise<{ data: any; ok: boolean }> {
...
server.registerTool(
  ...
  async ({ app }) => {
    try {
      ...
      if (!ok) return errorContent(data.error);
...
server.registerTool(
  ...
  async ({ app }) => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Failed to close application");
...
server.registerTool(
  ...
  async () => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Failed to fetch tools");
      return jsonContent(data);
    } catch (error) {
      return errorContent(String(error));
    }
...
server.registerTool(
  ...
  {
    description:
      "Execute a pentesting/CTF tool inside the Forge Kali environment and wait for the result. " +
    ...
    inputSchema: z.object({
      tool: z.string().describe(
        "Tool binary name to execute (e.g. 'nmap', 'gobuster', 'sqlmap', 'hydra', 'binwalk', 'curl')"
      ),
      args: z.array(z.string()).optional().describe(
        "Arguments to pass to the tool (e.g. ['-sV', '-p', '80,443', '10.10.10.1'])"
      ),
  ...
  async ({ tool, args, timeout }) => {
    try {
      const body: Record<string, any> = { tool };
      ...
      if (!ok) return errorContent(data.error ?? "Execution failed");
...
server.registerTool(
  ...
  {
    description:
      "Execute a pentesting/CTF tool in the background (for long-running scans). " +
    ...
    inputSchema: z.object({
      tool: z.string().describe(
        "Tool binary name to execute (e.g. 'nmap', 'masscan', 'hydra', 'gobuster')"
      ),
      args: z.array(z.string()).optional().describe(
        "Arguments to pass to the tool (e.g. ['-sV', '-p-', '10.10.10.0/24'])"
      ),
      timeout: z.number().optional().describe(
        "Maximum execution time in seconds (default: 300, max: 600)"
      ),
  ...
  async ({ tool, args, timeout }) => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Failed to start background job");
...
server.registerTool(
  ...
  async () => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Failed to list jobs");
...
server.registerTool(
  ...
  async ({ jobId }) => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Job not found");
...
server.registerTool(
  ...
  async ({ jobId }) => {
    try {
      ...
      if (!ok) return errorContent(data.error ?? "Failed to kill job");
